...
void gameLoop(struct Positions pos) {
	...
	int bufSize = 128;
	int c, numCon;
	char (*steps)[15] = malloc(sizeof *steps * 8); // tracks steps
	...
	while (!(strcmp(currentRoom, endRoom)) == 0) {
		...
		// printf("numCon is set to %d before infinite loop.\n", numCon); // Debug
		for(i = 1; i <= numCon; i++){
			fseek(f, 14, SEEK_CUR);
			fgets(str, 20, f);
			// remove newline
		...
		printf("\n");
		//printf("Makes it to 112\n"); // Debug
		
		//printf("Makes it to 114\n"); // Debug
		strcpy(steps[stepCount], currentRoom);
		//printf("Makes it to 116\n"); // Debug
		stepCount++;
		//printf("Makes it to 118\n"); // Debug
		fclose(f);
	}
